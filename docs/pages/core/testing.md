...
Always use `port: 0` when starting test servers. The OS assigns an available port automatically, preventing "address already in use" errors in CI or when running tests in parallel:
...
**Setting cookies:**
```typescript
import { cookies } from "@mage/cookies";
...
beforeAll(() => {
  ...
  server.app.get("/set-cookie", (c) => {
    c.cookie.set("session", "abc123");
    c.text("Cookie set");
  });
...
it("should set cookie", async () => {
  const response = await fetch(server.url("/set-cookie"));

  expect(response.headers.getSetCookie()).toContain("session=abc123");
  await response.text();
});
```
...
beforeAll(() => {
  ...
  server.app.get("/get-cookie", (c) => {
    const session = c.cookie.get("session");
    c.text(session ?? "none");
  });
...
it("should read cookie from request", async () => {
  const response = await fetch(server.url("/get-cookie"), {
...
beforeAll(() => {
  ...
  server.app.use(cookies({ secret: "test-secret" }));

  server.app.get("/set-signed", (c) => {
    c.cookie.setSigned("auth", "secret-value");
    c.text("Signed cookie set");
  });

  server.app.get("/get-signed", (c) => {
    const value = c.cookie.getSigned("auth");
    c.text(value ?? "invalid");
  });
...
it("should handle signed cookies", async () => {
  ...
  // Read signed cookie
  const getResponse = await fetch(server.url("/get-signed"), {
    headers: { Cookie: `auth=${signedCookie}` },
  });

  expect(await getResponse.text()).toBe("secret-value");
});
```
